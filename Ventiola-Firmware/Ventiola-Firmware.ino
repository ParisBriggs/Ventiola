// Vertiola firmware 
// Looking at Ventiola front on LHS and RHS is the orientation
// need to change Tools/Upload speed -> 115200 
// libraries to download: BLE-MIDI, MultiMap

// android apps to download: synprez fm 
// sound go to builtin 02, Bank INT, Patch 6 for Violin sound on synprez fm 
...
bool breath_sensor = false; // set to use breath sensor or not
...
// Pins 
const int softPotPinLHS = 15;
const int softPotPinRHS = 34; 
...
// Default is max velocity
int velocityToSend = 127;

// Breath sensor variables (used if variable above is set to true)
int velocityPrev = -2; 
int velocityMapped; 
int VelArraySize = 14; 
...
int pressureSensorADC;

void setup() {

  // Starts serial for printing
  if (debug) {
    Serial.begin(115200); 
  }
  // Sets breath sensor as input
  if (breath_sensor) {
    pinMode(pressureSensorPin, INPUT_PULLUP);
  }
  // Sets soft pot pins 
  pinMode(softPotPinLHS, INPUT_PULLUP);  
  pinMode(softPotPinRHS, INPUT_PULLUP); 
  // Begins MIDI mode
  MIDI.begin();
}
...
void loop() {
  // Read sensor values
  int softPotADC_LHS = analogRead(softPotPinLHS);
  int softPotADC_RHS = analogRead(softPotPinRHS);
  if (breath_sensor) {
  ...
  // Strings change by hitting space at the bottom of the ventiola 
  // (state move down with the LHS string and up with the RHS strings )
  if (softPotADC_LHS > 1000 ) {
    ...
    if (state != prev_state) {
      switch (state) {
        case GD:
          // Code for GD string configuration case
        ...
        case DA:
          // Code for DA string configuration case
        ...
        case AE:
          // Code for AE string configuration case
    ...
    // if the note has changed for RHS string or if the breath sensor is being used and the velocity has changed
    if (currNote_RHS != prevNote_RHS || (breath_sensor && velocityToSend != velocityPrev)) {
      // if a note is being pressed within the defined fretboard area
      if (softPotADC_RHS > 80 && softPotADC_RHS < 900 ) {
        // turn on the note
        MIDI.sendNoteOn(currNote_RHS, velocityToSend, 1);
      }
    } 
    // if the note has changed for LHS string or if the breath sensor is being used and the velocity has changed
    if (currNote_LHS != prevNote_LHS || (breath_sensor && velocityToSend != velocityPrev)) {
      // if a note is being played on LHS string within the defined fretboard area
      if (softPotADC_LHS > 140 && softPotADC_LHS < 945) {
        // turn on note on LHS string with velocity on channel 1
          MIDI.sendNoteOn(currNote_LHS, velocityToSend, 1);
        }
    }

    // note has changed for RHS string so turn off prev note
    if (prevNote_RHS != currNote_RHS) {
      // turn OFF prevNote RHS string with velocity on channel 2
      MIDI.sendNoteOff(prevNote_RHS, 0, 1);
    }

    // note has changed for LHS string so turn off prev note
  ...
  if (breath_sensor) {
    ...
    // delay is important for MIDI signals to send corectly
    delay(25);
  }
